...
namespace banggame {
    ...
    void equip_last_hero::on_enable(card_ptr origin_card, player_ptr target) {
        target->m_game->add_listener<event_type::on_destroy_card>(origin_card, [=](player_ptr origin, card_ptr e_origin_card, card_ptr target_card, destroy_flags &flags) {
            if (origin == target->m_game->m_playing && flags.check_all({ destroy_flag::intentional, destroy_flag::destroyed }) && target_card->pocket == pocket_type::player_table && target_card->is_blue()) {
