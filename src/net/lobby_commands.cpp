...
namespace banggame {
    ...
    void game_manager::register_chat_commands(game_lobby &lobby) {
        ...
        lobby.add_command({"kick", "KICK_DESCRIPTION", [](game_lobby &lobby, int user_id, std::string_view name_or_id) {
            lobby.m_mgr->kick_user_from_lobby(lobby.find_user(name_or_id).session);
        }}, command_permissions::lobby_owner);

        lobby.add_command({"mute", "MUTE_DESCRIPTION", [](game_lobby &lobby, int user_id, std::string_view name_or_id) {
            lobby.add_user_flag(lobby.find_user(name_or_id), game_user_flag::muted);
        }}, command_permissions::lobby_owner);
        
        lobby.add_command({"unmute", "UNMUTE_DESCRIPTION", [](game_lobby &lobby, int user_id, std::string_view name_or_id) {
            lobby.remove_user_flag(lobby.find_user(name_or_id), game_user_flag::muted);
        }}, command_permissions::lobby_owner);

        lobby.add_command({"appoint", "APPOINT_DESCRIPTION", [](game_lobby &lobby, int user_id, std::string_view name_or_id) {
        ...
        lobby.add_command({"reset-options", "RESET_OPTIONS_DESCRIPTION", [](game_lobby &lobby, int user_id) {
