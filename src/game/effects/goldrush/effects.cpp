...
namespace banggame {
    ...
    void effect_rum::on_play(card *origin_card, player *origin) {
        ...
        for (int i=0; i < 3 + origin->m_num_checks; ++i) {
            ...
            suits.push_back(origin->get_card_sign(origin->m_game->draw_card_to(pocket_type::selection, nullptr, show_card_flags::fast)).suit);
        }
        while (!origin->m_game->m_selection.empty()) {
            ...
            if (drawn_card->pocket == pocket_type::selection) {
                origin->m_game->move_card(drawn_card, pocket_type::discard_pile, nullptr, show_card_flags::fast);
            }
        }
