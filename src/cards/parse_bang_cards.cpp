...
void parse_effects(std::ostream &out, const YAML::Node &list, const std::string &name) {
    ...
    for (auto it = list.begin(); it != list.end();) {
        ...
        if (!target.empty()) {
            fmt::print(out, "          .target {{play_card_target_type::{}}},\n", target);
        }
...
void parse_all_effects(std::ostream &out, YAML::Node card) {
    try {
        fmt::print(out, "    {{\n      .name {{{}}}", std::quoted(card["name"].as<std::string>()));
        if (auto image = card["image"]) {
            fmt::print(out, ",\n      .image {{{}}}", std::quoted(image.as<std::string>()));
        }
...
int main(int argc, char **argv) {
    if (argc != 3) {
        fmt::print("Usage: {} bang_cards.yml bang_cards.cpp\n", argv[1]);
        return 1;
    }
