...
- **`request_base.h` / `request_queue.h/.cpp`** — The game proceeds via a **priority request queue** (`m_requests`, a `utils::stable_priority_queue`). Each `request_base` represents a pending action (e.g. "player X must respond with Missed!", "resolving a Duel"), and `request_queue` tracks one overall `request_state` — `done`, `next`, `waiting{timer}`, or `bot_play{origin, timer}` — describing what should happen next. The mechanism is split into two distinct halves, not a single per-tick step:
  - **`tick()`**, called once per server tick, only drives `invoke_tick_update()` — the *waiting* half. If the top request has an enabled `request_timer`, it counts that down (finishing it once expired); otherwise it just counts down whatever the current `waiting{timer}`/`bot_play{origin, timer}` state is, transitioning to `next` once a `waiting` timer reaches zero, or actually invoking `request_bot_play(origin, true)` once a `bot_play` timer reaches zero. If this resolves to `next`, `tick()` calls `commit_updates()`.
  - **`commit_updates()`** is the half that actually advances gameplay: it calls `invoke_update()` — which calls the top request's own `on_update()`, and, if the request itself didn't change, checks/restarts its timer and calls `request_bot_play(nullptr, false)` — in a tight loop, for as long as the result keeps coming back `next`. This lets a whole cascade of automatically-resolving requests (e.g. one card's effect immediately queuing another) play out synchronously within a single tick, bounded by two safety limits (`max_update_count = 30`, `max_update_timer_duration = 10s`) that force a `waiting` state instead of looping indefinitely.
  - `request_bot_play` sits at the end of `invoke_update()` almost unconditionally (not just when a bot's turn is detected up front) — it's the single place that decides, after every step, whether the next thing to do is hand control to a bot (scheduling `bot_play{origin, timer}`) or simply wait for a human response (`done`).
...
- **`bot_ai.cpp`** — Implements `game::request_bot_play`. `execute_random_play` takes the already-computed list of currently-playable cards (`play_cards`) and re-uses a single `std::vector<card_node>`, `node_set`, across every attempt: its capacity is reserved once up front (`play_cards.size() * repeat_card_nodes`, plus one for a possible null entry), so no attempt ever forces a reallocation. Each attempt (up to `bot_info.yml`'s `max_random_tries`) refills this same vector from scratch — every playable card pushed in `repeat_card_nodes` times (this duplication is what gives some candidates more chance of being picked than others), plus a null "do nothing" entry when responding to a timed request that allows it — and then drains it one candidate at a time: it repeatedly calls `get_selected_node` (`bot_rules.h/.cpp`, below) to pick a candidate's iterator out of the pool, swaps that element to the back of the vector and pops it off (an O(1) removal), and passes the underlying `playable_card_info` to `generate_random_play`, which fills in actual random targets for that specific card/modifier combination; `verify_and_play` then attempts it. An `ok` result advances the queue; a `prompt` or `error` result simply loops back to pick another candidate from whatever remains in that attempt's pool, since the one just tried has already been removed — once the pool empties without an `ok`, the next attempt begins and refills it. `bypass_prompt` (which forces a play through despite a non-empty `on_prompt` result, but never past a genuine `error`) follows two separate thresholds: from attempt index `bypass_empty_index` onward it applies to the very last candidate remaining in that pass (i.e. once every alternative in this attempt has already been tried), while from `bypass_unconditional_index` onward every attempt bypasses prompts regardless of what else is left. Running out of `max_random_tries` entirely without a successful `ok` is what's logged as a stalemate ("softlock").
- **`bot_rules.h/.cpp`** — Each rule is a small, stateless boolean predicate over one candidate `card_node`: `DEFINE_BOT_RULE` generates one struct per kind (`rule_filter_by_pocket`/`rule_filter_by_pocket_not` — which pocket a card is in; `rule_equip`; `rule_tag_value`/`rule_tag_value_not` — a tag present, absent, or matching a specific value; `rule_do_nothing` — matches only the null "pass" node). All but `rule_equip` judge the node through a shared helper, `get_first_card`: if the candidate play chains one or more modifiers ahead of a terminal card (`node->modifiers`), it's the *first* modifier's card that gets checked, not the terminal one — otherwise it's `node->card` itself. This is what lets a single generic rule correctly judge, say, a Bottle/Pardner-style choice card (the visible menu card played first) or a stacked-modifier play like *Aim* + *Bang!* (Aim itself) by the card the player actually reached for, rather than by whatever hidden or terminal card the chain resolves to. `rule_equip` is the one exception, checking `node->card` directly instead: a card played as a modifier can never itself be an equip card — only the terminal card of a chain can be one — so the equip check is meaningful only against that terminal card, regardless of whatever modifiers precede it. `bot_info.yml` sequences these predicates into two ordered lists, `in_play_rules`/`response_rules`, compiled by `config/parse_bots.py` into the `bot_settings` spans `bot_ai.cpp::get_selected_node` walks — in order, taking the first rule that matches *anything* still in the pool and picking uniformly at random among just those matches, falling back to a uniform pick over everything remaining if no rule matches at all. The base game's actual `in_play_rules`, for instance, are ordered `tag_value(strong)` > `equip` > `pocket(player_table)` > `pocket(player_hand)` > `pocket(player_character)` > `tag_value_not(pass_turn)` — so, concretely, *Indians!* (`strong(1)`) or *Wells Fargo* (`strong(3)`) get preferred over an ordinary hand/table play whenever either is in the pool alongside one.
- **`bot_suggestion.h/.cpp`** — The heuristics bots use to guess *who's on their side* in a hidden-role game where a player's actual role often isn't known. It tracks a lightweight, mutually-exclusive "karma" flag (`positive`/`negative`/neutral) per unknown player, nudged by `signal_hostile_action`/`signal_helpful_action` every time a bot observes someone attack or help someone else (`signal_remove_card` is a convenience wrapper that treats destroying a card as hostile, unless that card is a `penalty`-tagged one like Jail — removing that is actually helpful): when the *target* of an observed action has a known role, the *acting* player's karma is set directly from simple Bang! alignment rules (attacking a known lawman reads as bandit-like, attacking a known outlaw/renegade reads as lawman-like); when the target's role is itself still unknown, karma propagates transitively through a small fixed lookup table instead. Both signal functions bail out immediately, before touching karma at all, when the observed action carries `effect_flag::target_players` — an effect that hits every player at once (Gatling, Indians!) reveals nothing about the origin's hidden alignment the way deliberately choosing one specific victim would, so the karma model only ever updates off genuinely chosen, single-target attacks and heals. `is_target_enemy`/`is_target_friend` then combine a bot's own (always-known) role, whatever's actually been revealed, and this accumulated karma into a real verdict, implementing Bang!'s full alignment logic per role — including the Renegade's context-dependent flip (siding with whichever faction, law or outlaws, currently has fewer members left) and the 3-player and free-for-all variants. These two functions are the actual reasoning behind every `bot_check_target_friend`/`bot_check_target_enemy` call in `prompts.cpp`.
- **Reconciling several `bot_check_*` verdicts on one multi-target card.** Every `bot_check_*` helper in `prompts.cpp` judges exactly one target at a time. A single-`player` card only ever needs one such verdict, but a `players`-target card — e.g. Gatling (`bang players alive notself`, hitting every `alive`, non-self player at once; `target_types/base/players.cpp`) — calls the same per-target checks once per player currently in range and then has to fold all of them into the *one* `prompt_string` its `on_prompt` needs to return, since there's no single target left to ask "should I avoid this one?" about. That fold is `select_prompt_fallback_empty` (§3.4): it keeps the highest-priority non-empty result among all the per-target verdicts, but discards it if its priority is only the default (0) and at least one *other* target's verdict came back empty. Concretely, for a Sheriff bot's Gatling that would hit both an enemy Outlaw and its own Deputy: `bot_check_target_enemy`'s plain priority-0 "not an enemy" complaint about the Deputy gets forgiven, because the Outlaw target came back clean — Gatling can't be aimed around the Deputy the way a single-target Bang! could just pick the Outlaw instead, so one genuine enemy anywhere in range is enough to justify the whole play. But `bot_check_kill_sheriff`'s priority-1 "this would finish off your own Deputy" (triggered only if the Deputy is at 1 HP) survives regardless of the Outlaw also being in range, since that specific bad outcome is just as unavoidable as the enemy is legitimate. Either way the result is invisible to human players and only ever changes which candidate `execute_random_play` tries next.
